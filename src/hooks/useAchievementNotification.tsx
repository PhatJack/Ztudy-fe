...
export function useAchievementNotification() {
  ...
  useEffect(() => {
    ...
    const handleAchievementEvent = (event: CustomEvent) => {
      ...
      setAchievementMessage(message || `You've reached level ${level}!`);
      setShowAchievement(true);
    };
    
    // Add a custom event listener for achievement notifications
    window.addEventListener('achievement-notification', handleAchievementEvent as EventListener);
    
    return () => {
      window.removeEventListener('achievement-notification', handleAchievementEvent as EventListener);
    };
  }, [connectOnlineSocket]);
