...
const Sidebar = () => {
  return (
    <div className="md:max-w-28 h-screen sticky top-0 p-2 box-border flex flex-col justify-start items-center gap-4 bg-white">
      ...
      <ul className="w-full flex flex-col gap-1">
        {menu.map((item, index) => (
          <li key={index} title={item.label}>
            <Link
              href={item.href}
              className="w-full flex flex-col justify-center items-center gap-1 rounded-md hover:bg-secondary/50 transition-all p-2 cursor-pointer"
            >
              <span>
