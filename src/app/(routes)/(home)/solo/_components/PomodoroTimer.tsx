...
const PomodoroTimer: React.FC = () => {
  ...
  const enterPip = async () => {
    ...
    if (!state.backgroundURL || !UrlToEmbeded(state.backgroundURL)?.videoId) {
      console.error("Invalid or missing YouTube URL");
      return;
    }

    const videoId = UrlToEmbeded(state.backgroundURL)?.videoId;
    ...
    if (!pipWindowRef.current) {
      console.error(
        "Failed to open PIP window. Check if popup blockers are enabled."
      );
      return;
    }
  ...
  const updatePipWindow = () => {
    if (pipWindowRef.current && !pipWindowRef.current.closed) {
      ...
      const timeString = `${String(currentFormattedTime.hours).padStart(2, '0')}:${
        String(currentFormattedTime.minutes).padStart(2, '0')}:${
        String(currentFormattedTime.seconds).padStart(2, '0')}`;
  ...
  return (
    <>
      <TooltipTemplate content="Pure in Picture Mode">
        <Button
